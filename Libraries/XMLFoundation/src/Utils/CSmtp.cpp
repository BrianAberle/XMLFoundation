...
#ifndef _WIN32 // we need to define OutputDebugStringA to something on Linux
	//#define OutputDebugStringA(x) cout<<x;
	#define OutputDebugStringA(x) 
#endif


//--UBT removed pragma-link
	//#pragma comment(lib, "ssleay32.lib")
	//#pragma comment(lib, "libeay32.lib")

//
// and UBT added this pragma-link.....
			...
			#  if defined(_MSC_VER) && _MSC_VER < 1900 // for VS2003 VS2005 VS2012 and VS2013 
				...
				//	while every other version of MS Compiler supportss XMLFoundation being unzipped to anywhere and everything will compile.
				// Looks like a bug to me.
				#pragma comment(lib,    "C:/XMLFoundation/Libraries/openssl/bin-win64/libeay32.lib")	
				#pragma comment(lib,    "C:/XMLFoundation/Libraries/openssl/bin-win64/ssleay32.lib")
			#endif
		#elif __WINPHONE
...
//	https://social.msdn.microsoft.com/Forums/vstudio/en-US/4a1c9610-fa41-45f6-ad39-c9f6795be6f2/msvcrt-iob-disappeared?forum=vclanguage#page:2
	...
	// Strangely, _iob is missing to slightly different degrees in various environments.
	#  if defined(_MSC_VER) && _MSC_VER < 1900 // for older than VS2015
	...
	#else
		// building with VS2015, the compilable syntax for an _iob definition changed and the size grew from 3 to 20.
		// In addition to _iob, in VS2015 we must also supply __iob_func() per dependencies from (libeay32.lib(cryptlib.obj))
		_iobuf _iob[20];
		extern "C" FILE * __cdecl __iob_func(void)	{	return _iob;	}

		// also, only for VS2015 , _vsnprintf() as gone missing from the C Runtime

		// https://social.msdn.microsoft.com/Forums/vstudio/en-US/e931fc51-d025-400d-9ee8-aff2c9514c96/error-c3163-vsnprintf?forum=vclanguage

		// In VS2015 if the following is uncommented
//		extern "C" int _vsnprintf(char* str, size_t size, const char* format, va_list ap)		{		return vsnprintf(str, size, format, ap);  	}
		// compile error C2084 : function 'int _vsnprintf(char *const ,const size_t,const char *const ,va_list)' already has a body

		// and it we leave it commented out, thelibrary compiles fine, the application compiles then
		// any application that links to openssl(only in VS2015), such as the example program "WebServerServices" or "SMTPandTLS" produce the following LINK errors:
		// 2>libeay32.lib(cryptlib.obj) : error LNK2019 : unresolved external symbol _vsnprintf referenced in function OPENSSL_showfatal
		// 2>libeay32.lib(cryptlib.obj) : error LNK2019 : unresolved external symbol _vsnwprintf referenced in function OPENSSL_showfatal
		//
		//
		// Additionally, to _vsnprintf() gone missing as the above two forums have identified, in VS2015 fprintf() has gone missing (but only in the release build)
		// someone should start a thread about it so microsoft knows.  Using VS2013 or 2012 is a workaround.
